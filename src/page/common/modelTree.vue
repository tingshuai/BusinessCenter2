...
<div class="lTree">
    ...
    <div class="treeBox qwscroll" :style="{height:sidebarHeight-170+'px'}">
    <el-tree  ref="lTree"
        :props="config.propsTree"
        :style="{minHeight:sidebarHeight-270+'px',width:(320+ (config.maxLevel>1 ? (config.maxLevel-3)*24 : 0)) + 'px'}"
        ...
        <span class="custom-tree-node" style="display:block;" slot-scope="{ node, data }">            
            ...
            <span>
                ...
                <el-button v-if="data.id>0" type="text" size="mini" @click.stop="() => edit(node, data)">
                    <img class="tree_after_icon edit" :src="emptySrc" alt="编辑">
                </el-button>              
                <el-button v-if="data.id>0" type="text" size="mini"  @click.stop="() => remove(node, data)">
            ...
            setTimeout(() => {
                ...
                center: true
                    ...
                    warehouseDel({Vue:this,params:params}).then(res=>{
                        if(res.result){                         
                            this.$emit("delNode",{self:this,node:node,nodeData:data})
