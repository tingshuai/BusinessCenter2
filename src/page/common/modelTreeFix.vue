<template>
<div class="lTree" :class="styles.boxClass" :style="styles.boxStyle">
    <el-form class="treeForm" style="padding:14px;box-sizing:border-box;" :style="styles.treeFormStyle" >
        <el-select v-show="styles.showSlect" @change="treeStatusChange" class="treeSelect" v-model="config.treeFilter.status">
            <el-option v-for="(item,i) in config.dropDown.treeStatus" :label="item.label" :value="item.value" :key="i"></el-option>
        </el-select>  
        <el-input placeholder="请输入关键字" class="treesearch" @change="treeStatusChange" @input="treeStatusChange" v-model="config.treeFilter.keyWords">
            <el-button slot="append" @click="treeStatusChange">搜索</el-button>
        </el-input>    
    </el-form>
    <div class="treeBox qwscroll" :style="styles.treeBox? styles.treeBox :{ height:((sidebarHeight) * 0.6)-170+'px'}">
    <el-tree  ref="lTree"
        :props="config.propsTree"
        :style="styles.lTree? styles.lTree:{minHeight:((sidebarHeight-270) * 0.5)+'px',width:(320+ (config.maxLevel>1 ? (config.maxLevel-3)*24 : 0)) + 'px'}"
