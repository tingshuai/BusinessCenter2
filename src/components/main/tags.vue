...
<div class="tags_root" ref='tags_root'>
	<ul ref="tags_box" class="list_box" :style="'width:'+ (170* tags.length+20)+'px'">
		<li v-for="(item,i) in tags" @click.stop="clickTag(i)" :class="tagsActiveIndex==i? 'active':''"><i :class="i==0? 'qw_icon_home' : 'qw_icon_page'"></i> {{item.title}} <span v-if="i>0" @click.stop="closeTag(i)" class="el-icon-error"></span></li>
	</ul>
	<div class="qw_tool_box">
		<el-dropdown style="margin:6px 0 0 11px;">
		  <el-button size="mini" type="primary">
		    更多操作
		  </el-button>
		  <el-dropdown-menu slot="dropdown" style="width:110px;">
		    <el-dropdown-item><span  @click="closeOther">关闭其他</span></el-dropdown-item>
...
var vueObj={
	...
	created(){
		that=this;
		// this.ADD_TAG({title:'首页',path:'/Desktop'})
	},
	computed:{
		...mapState({
	...
	watch:{
		tagsActiveIndex: (new_,old_)=>{
			console.log("activeIndexChanged");
